class KitchenItemsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @kitchen_item.save
        ...
        format.json { render action: 'show', status: :created, location: @kitchen_item }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to kitchen_items_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def kitchen_item_params
      params.require(:kitchen_item).permit(:kitchen_menu_item_id, :ticket_id, :kitchen_closeout)
    end
end
